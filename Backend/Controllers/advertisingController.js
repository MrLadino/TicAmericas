...
exports.getAdvertising = async (req, res) => {
  try {
    ...
    // Seleccionar solo las categorías asociadas al usuario
    const [categories] = await db.query(
      "SELECT * FROM advertising_categories WHERE user_id = ?",
      [userId]
    );
    // Para cada categoría, buscar sus archivos en advertising_files
...
// Elimina la categoría (y sus archivos si la FK tiene ON DELETE CASCADE)
exports.deleteCategory = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Categoría eliminada correctamente", category_id: id });
  } catch (error) {
...
// Sube múltiples archivos y los asigna a una categoría
exports.uploadFiles = async (req, res) => {
  ...
  if (!req.files || req.files.length === 0) {
    return res
      .status(400)
      .json({ message: "No se subió ningún archivo o los archivos no son válidos." });
  }
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Archivos subidos correctamente", total: req.files.length });
  } catch (error) {
...
// Elimina un archivo específico (imagen o video)
exports.deleteFile = async (req, res) => {
  const userId = req.user.user_id;
  const { file_id } = req.params;

  try {
    // Verificar que el archivo pertenezca a una categoría de este usuario
    const [rows] = await db.query(
      `SELECT f.file_id, c.user_id
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Archivo no encontrado o no pertenece al usuario." });
    }
